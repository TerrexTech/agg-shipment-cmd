...
var _ = Describe("ResultTest", func() {
	...
	Describe("AddItem", func() {
		It("should create AddItem response", func(done Done) {
			...
			mockCmd := createCmd(producer.Input(), "AddItem", reqTopic, respTopic, marshalItem)
			...
			msgCallback := func(msg *sarama.ConsumerMessage) bool {
				...
				if doc.CorrelationID == mockCmd.UUID {
					...
					err = json.Unmarshal(doc.Data, itemModel)
					Expect(err).ToNot(HaveOccurred())
	...
	Describe("DeleteItem", func() {
		It("should create DeleteItem response", func(done Done) {
			...
			mockCmd := createCmd(producer.Input(), "DeleteItem", reqTopic, respTopic, marshalItem)
	...
	Describe("UpdateItem", func() {
		It("should create UpdateItem response", func(done Done) {
			...
			mockCmd := createCmd(producer.Input(), "UpdateItem", reqTopic, respTopic, marshalParams)
